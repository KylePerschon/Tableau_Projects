...
def rename_duplicate_rows(df, column_name):
...
def test_x_y(data_frame: pd.DataFrame, start_point:str, _x:int = 1, _y:int=1, _data:list = None):
    ...
    if levels_count == 1:
        pass
    elif levels_count %2 == 0:
        _y = _y-levels_count/2
    else:
        numbers_list = list(range(1, levels_count+1))
        _y = _y - int(np.median(numbers_list))
    for i, row in temp_df.iterrows():
        next_start_point = row['start_point']
        node_cords = {'node':next_start_point, 'x_cord':_x, 'y_cord':(_y+i), 'target':start_point, 'node_cluster_count': levels_count, 'node_cluster_list': node_cluster_list, 'branch_order': i+1}
        if _data:
            _data.append(node_cords)
        else:
            _data = [node_cords]
        test_x_y(data_frame=data_frame, start_point=next_start_point, _x=_x+1, _y=_y+i, _data=_data)
    return _data

def total_nodes_in_branch(data_frame: pd.DataFrame, start_point:str):
    temp_df = data_frame[data_frame['end_point'] == start_point].reset_index()
    total_count = len(temp_df) if len(temp_df) > 0 else 1
    down_stream_cnt = 0
    for i, row in temp_df.iterrows():
        next_start_point = row['start_point']
        max_down_nodes = total_nodes_in_branch(data_frame=data_frame, start_point=next_start_point)
...
def assign_y_values(data_frame:pd.DataFrame, start_point:str, min_range:int, max_range: int, y_values_list: list = None):
    y_val = get_y_from_range(min_range=min_range, max_range=max_range)
...
def get_y_from_range(min_range:int, max_range:int) -> float:
    return (max_range+min_range)/2


for i, sp_row in start_points_df.iterrows():
    original_start_point = sp_row['start_point']
    result = test_x_y(data_frame=node_edge_df, start_point=original_start_point)
    ...
    for i, row in df.iterrows():
        ...
        check_max = total_nodes_in_branch(node_edge_df, start_point)
